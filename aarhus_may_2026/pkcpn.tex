...
\begin{frame}
	\begin{theorem}[dB-Panov, 2025-26]
		Let \(H_1, \ldots, H_n\) be complex hyperplanes in \(\CP^d\) with \(d \geq 1\) and let \(a_1, \ldots, a_n\) be real numbers in the open interval \((0,1)\). Suppose that the weighted arrangement \(\{(H_i, a_i)\}_{i=1}^n\) satisfies the following conditions:
		\begin{enumerate}
			\item \(\sum_{i=1}^{n} a_i = d+1\), i.e., the log pair \((\CP^d, \sum a_i H_i)\) is Calabi-Yau;
			\item \textbf{stable}, i.e., the log pair \((\CP^d, \sum a_i H_i)\) is klt.
		\end{enumerate}
		Then the following inequality holds:
		\begin{equation}\label{eq:my}
		Q(a_1, \ldots, a_n) \leq 0 	
		\end{equation}
		where \(Q\) is the \textbf{Hirzebruch quadratic form} of the hyperplane arrangement \(\mH = \{H_1, \ldots, H_n\}\). 
		Moreover, the equality holds in \eqref{eq:my} if and only if there exists a \textbf{polyhedral K\"ahler metric} on \(\CP^d\) with cone angles \(2\pi\alpha_i\) along the hyperplanes \(H_i\), where \(\alpha_i = 1-a_i\).
	\end{theorem}
\end{frame}
