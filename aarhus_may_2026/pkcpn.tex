...
\begin{frame}{Outline}
	\begin{itemize}
		...
		\item Introduction: flat metrics on surfaces with conical singularities
...
\begin{frame}{The \(2\)-cone}
	Let \(\alpha>0\). Denote by \(C(2\pi\alpha)\) the cone over a circle of length \(2\pi\alpha\). 
	\\
	...
	\begin{center}
		\scalebox{0.7}{	
			...
			\node[scale=1.3] at (6.8,.8) {\(r\)};
			\node[scale=1] at (5.5,2.7) {\((2\pi\alpha) r\)};
...
\section{surfaces}
\begin{frame}{Flat metrics on surfaces with conical singularities}
	\begin{itemize}
		\item Let \(\Sigma\) be a compact orientable \(2\)-dimensional surface.
		
		\item A flat metric on \(\Sigma\) with conical singularities is a flat metric on \(\Sigma \setminus \{p_1, \ldots, p_n\}\) that is locally isometric to \(C(2\pi\alpha_i)\) near \(p_i\)
...
\begin{columns}
	...
	\begin{column}{.48\textwidth}
		\begin{figure}
			...
			\caption{Double of a triangle}
		\end{figure}
	\end{column}
\end{columns}

\begin{center}
\textbf{Gauss-Bonnet:} \(\sum_{i=1}^{n}(1-\alpha_i) = \chi(\Sigma)\)	
\end{center}
\end{frame}

\begin{frame}{Riemann surfaces}
	\begin{itemize}
		\item In 2D: metric + orientation \(\implies\) complex structure
		\item The complex structure extends smoothly across conical singularities: if \(z=r^{1/\alpha}e^{i\theta}\) then
		\[
		dr^2 + \alpha^2 r^2 d\theta^2 = \alpha^2 |z|^{2\alpha-2} |dz|^2
		\]
	\end{itemize}
\end{frame}
...
\begin{frame}{McOwen (1988), Troyanov (1991)}
	...
	\begin{itemize}
		...
		\[
		\forall i \quad
		a_i < \frac{1}{2} \cdot \sum_{j=1}^{n} a_j  \quad \textup{ where } a_i = 1-\alpha_i
		\]
	\end{itemize}
\end{frame}
...
\begin{frame}{Main result}
	\begin{theorem}[dB-Panov, 2025-26]
		...
		Moreover, the equality holds if and only if there exists a \textbf{polyhedral K\"ahler metric} on \(\CP^d\) with cone angles \(2\pi\alpha_i\) along \(H_i\).
	\end{theorem}
\end{frame}
...
\begin{frame}{Hyperplane arrangements: basics and notation}
	\begin{itemize}
		\item A \textbf{hyperplane arrangement} is a finite set \(\mH = \{H_1, \ldots, H_n\}\) of pairwise distinct complex hyperplanes \(H_i \subset \CP^d\)
		\item Index set \([n]=\{1, \ldots, n\}\)
		\item The \textbf{intersection poset} of \(\mH\) is the set of all proper linear subspaces obtained by intersecting hyperplanes in \(\mH\)
		\[
		\mL = \big\{L \subset \CP^d \,\mid\, L = \bigcap_{i \in I} H_i \textup{ for some non-empty } I \subset [n]\big\}
		\]
		\item The \textbf{rank} of \(L \in \mL\) is defined as
		\[
		r(L) = \codim (L \subset \CP^d) 
		\]
		Take the convention that \(r(\emptyset) = d+1\)
		\item \(\mL^k = \{L \in \mL \,\mid\, r(L) =k\}\)
		\item The \textbf{multiplicity} of an element \(L \in \mL\) is defined as
		\[
		\mul(L) = \big| \{i \,|\, H_i \supset L\} \big|
		\]
	\end{itemize}
\end{frame}

\begin{frame}{Stability}
	A weighted arrangement \(\{(H_i, a_i)\}_{i=1}^n\) consisting of hyperplanes \(H_i \subset \CP^d\) together with weights \(a_i>0\) is \textbf{stable} if 
	\[
	\forall L \in \mL \setminus \{\emptyset\}: \quad
	\frac{1}{r(L)} \cdot \sum_{i \mid H_i \supset L} a_i < \frac{1}{d+1} \cdot \sum_{i=1}^{n} a_i
	\]
	Suppose that 
	\begin{equation}\label{eq:cy}
		\sum_{i=1}^{n} a_i = d+1	
	\end{equation}
	Then stability is equivalent to
	\begin{equation}\label{eq:klt}
		\forall L \in \mL \setminus \{\emptyset\}: \quad	\sum_{i \mid H_i \supset L} a_i < r(L)
	\end{equation}
	Let \((X, \Delta)\) be the log pair \((\CP^d, \sum a_i H_i)\). 
	\[
	\begin{aligned}
		\textup{Equation \eqref{eq:cy}} &\iff (X, \Delta) \textup{ is Calabi-Yau} \\
		\textup{Equation \eqref{eq:klt}} &\iff (X, \Delta) \textup{ is klt}
	\end{aligned}
	\]
...
\section{hirzebruch qf}
\begin{frame}
	\frametitle{The Hirzebruch quadratic form}
	Let \(\mH = \{H_1, \ldots, H_n\}\) be a hyperplane arrangement in \(\CP^d\).
	The \textbf{Hirzebruch quadratic form} of \(\mH\) is the homogeneous degree \(2\) polynomial on \(\R^n\) given by
		\[
	Q(x_1, \ldots, x_n) = \sum_{i,j = 1}^k Q_{ij} x_i x_j 
	\]
	\[
	\mathcolorbox{green!20}{
		Q_{ij} = \begin{cases}
			- (d+1) \sigma_i + 2d &\text{ if } i = j  \\
			-2 &\text{ if }  i \neq j \text{ and } \mul \big(H_i \cap H_j\big) = 2  \\
			\,\, d-1 &\text{ if } i \neq j \text{ and } \mul \big(H_i \cap H_j\big) \geq 3 
		\end{cases}
	}
	\]
	where \(\sigma_i = \left| \{L \in \mL^2 \,|\, L\subset H_i \,,\, \mul(L) \geq 3 \}\right|\) 
\end{frame}
