...
\begin{frame}
	...
	\begin{itemize}
		...
		\pause
		\item If \(\chi(X, \bm{\alpha}) > 0\). Furthermore, assume that \(\alpha_i \in (0,1)\) and \(n \geq 3\). Then there exists a unique  K\"ahler metric on \(X = \CP^1\) with \(K_g = 1\) and cone angle \(2\pi\alpha_i\) at \(x_i\) if and only if
		\[
		\forall i \quad
...
\begin{frame}
	...
	Follow Panov (2009) who proved the theorem for \(d=2\) 
	\begin{itemize}
		\pause
